...
/** Serializable target for post-generation navigation (no function props from RSC → client). */
export type AfterCreateRedirect = "teacher-quizzes" | "student-take";

export type CreateQuizWizardProps = {
  heading: string;
  subheading?: string;
  afterCreateRedirect: AfterCreateRedirect;
};

export function CreateQuizWizard({ heading, subheading, afterCreateRedirect }: CreateQuizWizardProps) {
  ...
  async function handleGenerate() {
    ...
    try {
      ...
      const res = await fetch("/api/generate", {
        ...
        body: JSON.stringify({
          fileId,
          title: (title || file?.name?.split(".")[0]) ?? "Untitled Quiz",
          config,
        }),
      });
      const data = await res.json();
      ...
      const quizId = data.quiz?.id as string | undefined;
      if (!quizId) {
        toast.error("Quiz was created but the response was incomplete.");
        setStep("configure");
        return;
      }
      if (afterCreateRedirect === "teacher-quizzes") {
        router.push("/teacher/quizzes");
      } else {
        router.push(`/student/take/${quizId}`);
      }
    } catch {
  ...
  return (
    <div className="max-w-2xl mx-auto flex flex-col gap-6">
      <div>
        ...
        {subheading ? <p className="text-muted-foreground text-sm mt-1">{subheading}</p> : null}
      </div>
      ...
      {step === "configure" && (
        <Card>
          ...
          <CardContent className="flex flex-col gap-5">
                ...
                {DIFFICULTIES.map((d) => (
                  <button
                    ...
                    className={`rounded-lg border p-3 text-left transition-colors ${
                      difficulty === d.value ? "border-primary bg-primary/5" : "hover:bg-muted/60"
                    }`}
                  >
            ...
            <div className="flex flex-col gap-1.5">
              <Label>
                Time limit (minutes) <span className="text-muted-foreground font-normal">— 0 for unlimited</span>
              </Label>
              <Input
                type="number"
                min={0}
